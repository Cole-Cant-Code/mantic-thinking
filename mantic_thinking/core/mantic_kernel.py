...
Where:
    ...
    I: Array of interaction coefficients (0.1-2.0)
...
def mantic_kernel(W, L, I, f_time=1.0, k_n=1.0):
    ...
    Args:
        ...
        I: array of 4 interaction coefficients (0.1-2.0)
        f_time: temporal kernel value (default 1.0)
        k_n: normalization constant (default 1.0)
    ...
    # Interaction coefficients are bounded but may exceed 1.0 (amplification) in v1.4.0+.
    if np.any((I < 0.1) | (I > 2.0)):
        raise ValueError("Interaction coefficients (I) must be in range [0.1, 2.0]")
